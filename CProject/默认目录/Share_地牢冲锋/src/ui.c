...
bool moveCursor(int optsNum) {
  ...
  while (SDL_PollEvent(&e)) {
    if (e.type == SDL_QUIT) {
      ...
      cursorPos = optsNum;
      return quit;
    } else if (e.type == SDL_FINGERMOTION  || e.type == SDL_FINGERUP || e.type == SDL_FINGERDOWN) {
      int nowSecond = time((time_t*) NULL);

      int keyValue = SDLK_UNKNOWN;
      if(e.tfinger.dy >= 0.01 || e.tfinger.dy <= -0.01 || e.tfinger.dx >= 0.01 || e.tfinger.dx <= -0.01){
        if(e.tfinger.dy < 0){
          if(e.tfinger.dx < 0){
            if((e.tfinger.dx - e.tfinger.dy) > 0.01){
              keyValue = SDLK_UP;
            }else{
              keyValue = SDLK_LEFT;
            }
      ...
      if(e.type == SDL_FINGERUP){
          if((e.tfinger.x - lastX) < 0.02 || (e.tfinger.y - lastY) < 0.02){
              keyValue = SDLK_RETURN;
          }
      }
      ...
      ALOGD("keyValue %d %f %f now %d last %d", keyValue , e.tfinger.dy, e.tfinger.dx, nowSecond, lastSecond);
