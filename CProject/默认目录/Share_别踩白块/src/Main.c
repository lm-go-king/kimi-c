...
int download(Data *data)
{
    ...
    if(!(sockfd=link2("pop.163.com",110)))
        return 0;
    char buff[2000];
    char *p;
...
{
    int sockfd;
    if(!(sockfd=link2("smtp.163.com",25)))
        return 0;
    char buff[2000];
    char *p;
    //登录邮箱
    ...
    for(int i=0; i<5; i++)
    {
        ...
        {
        case 0:
            p="MAIL FROM: <xxxxxx@163.com>\r\n";
            break;
        case 1:
            p="RCPT TO: <xxxxxx@163.com>\r\n";
            break;
        case 2:
            p="DATA\r\n";
            break;
        case 3:
            strcpy(buff,"SUBJECT:111\r\n\r\n");
            for(int i=0; i<2; i++)
                for(int j=0; j<20; j++)
                {
                    char s[50];
